...
class SignatureManager(object):
    def __init__(self):
        ...
        self.encode_defunct = getattr(import_module("eth_account.messages"), "encode_defunct")
        self.verified = 0
        self.signed = 0
    
    def signTransaction(self, private_key, transaction):
        message = self.encode_defunct(text=transaction["data"])
    ...
    def verifyTransaction(self, transaction):
        message = self.encode_defunct(text=transaction["data"])
        _hash = self.w3.soliditySha3(["string"], [transaction["data"]]).hex()
        _hashInTransaction = transaction["hash"]
        signer = self.w3.eth.account.recover_message(message, signature=transaction["sig"])
        sender = self.w3.toChecksumAddress(json.loads(transaction["data"])["from"])
